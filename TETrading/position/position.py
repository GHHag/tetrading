...
class Position:
    """
    Handles entering and exiting a position in an asset and holds
    ...
    fixed_position_size : Keyword arg 'Boolean'
    ...
    def __init__(self, capital, fixed_position_size=True, roundtrip_pct_cost=0.0):
        ...
        self.__position_return = None
        self.__profit_loss = None
        self.__mae = None
        self.__mfe = None
        self.__returns_list = []
    ...
    def enter_market(self, entry_price, direction, entry_dt=None):
        ...
        if direction not in ['long', 'short']:
            raise ValueError('Direction of position specified incorrectly, make sure it is a string with a value of '
                             f'either "long" or "short".\n Given value: {direction}')
    ...
    def exit_market(self, exit_price, exit_signal_dt=None):
        ...
        :param exit_signal_dt:
            'Pandas Timestamp/Datetime' : Time and date when exiting
            the market.
        :return:
            'int/float/Decimal' : Returns the capital amount, which is
            the same as the Position was instatiated with if
    ...
    def _unrealised_profit_loss(self, current_price):
        ...
        if self.__direction == 'long':
            self.__unrealised_profit_loss = Decimal(current_price - self.__entry_price).quantize(Decimal('0.02'))
            self.__position_profit_loss_list.append(self.__unrealised_profit_loss)
        elif self.__direction == 'short':
            self.__unrealised_profit_loss = Decimal(self.__entry_price - current_price).quantize(Decimal('0.02'))
            self.__position_profit_loss_list.append(self.__unrealised_profit_loss)
    ...
    def _unrealised_return(self, current_price):
        ...
        if self.__direction == 'long':
            ...
            self.__market_to_market_returns_list.append(
                Decimal((current_price - self.__last_price) / self.__last_price * 100).quantize(Decimal('0.02'))
            )
            ...
            self.__market_to_market_returns_list.append(
                Decimal((self.__last_price - current_price) / self.__last_price * 100).quantize(Decimal('0.02'))
            )
            self.__returns_list.append(unrealised_return)
            self.__last_price = current_price
            self.__unrealised_return = unrealised_return
    ...
    def print_position_stats(self):
        ...
        print(f'Unrealised P/L sequence: {list(map(float, self.__position_profit_loss_list))}\n'
