...
class SafeFPositionSizer(PositionSizer):
    ...
    def __call__(self, best_estimate_positions, period_len,
        ...
        Parameters
        ----------
        :param best_estimate_positions:
            'List' : A sequence of positions.
        :param period_len:
        ...
        if len(best_estimate_positions) >= forecast_positions:
            split_data_fraction = forecast_positions / len(best_estimate_positions)
        ...
        # get the drawdown value at the percentile set to be the threshold at which to limit the probability of getting
        # a max drawdown of that magnitude at when simulating sequences of the best estimate positions
