...
def candlestick_plot(df_og, entry_date, entry_price, exit_date, exit_price, save_fig_to_path=None):
    """
    ...
    plot_style = mpf.make_mpf_style(base_mpf_style='yahoo', gridstyle='', facecolor='#4C5063', marketcolors=mc)
    ap0 = [mpf.make_addplot(df[['Entry_price']], scatter=True, marker='^', color='green', markersize=75),
           mpf.make_addplot(df[['Exit_price']], scatter=True, marker='v', color='red', markersize=75)]

    if save_fig_to_path:
        mpf.plot(df, type='candle', style=plot_style, volume=True, main_panel=0, volume_panel=1,
